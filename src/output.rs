...
fn rule_info(rule_id: &str) -> Option<RuleInfo> {
    Some(match rule_id {
        ...
        "DEPSEC-P024" => RuleInfo { name: "Pickle Deserialization", narrative: "Calls pickle.loads(), pickle.load(), or pickle.Unpickler() to deserialize data. Pickle deserialization executes arbitrary code embedded in the serialized data. Never deserialize untrusted pickle data." },
        "DEPSEC-P025" => RuleInfo { name: "WebAssembly Binary", narrative: "Contains a WebAssembly (.wasm) binary file. WASM in npm packages is unusual — 75% of WASM samples in the wild are malicious (CrowdStrike). WASM hides logic in compiled binary format, evading text-based scanners." },
        "DEPSEC-COMBO-001" => RuleInfo { name: "Cross-File Exfiltration", narrative: "This package reads sensitive credentials in one file and has network or exec capability in a different file. This is the classic scatter attack pattern: split malicious logic across multiple files so no single file triggers detection." },
        "DEPSEC-COMBO-002" => RuleInfo { name: "Cross-File Dropper", narrative: "This package has code execution in one file and obfuscation patterns in another file. Attackers use obfuscation to hide the payload and a separate clean-looking file to execute it." },
        "DEPSEC-COMBO-003" => RuleInfo { name: "Cross-File Obfuscated Loader", narrative: "This package has dynamic module loading (require with variable) in one file and obfuscation in another. Common in staged malware that loads payloads at runtime." },
        // Ruby rules
        ...
        "DEPSEC-P034" => RuleInfo { name: "Ruby Pipe Execution", narrative: "Uses open(\"|\") to spawn a shell command via Kernel#open. When the command argument comes from user input, this enables command injection." },
        // Rust rules
